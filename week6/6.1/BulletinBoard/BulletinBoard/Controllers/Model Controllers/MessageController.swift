...
class MessageController {
    ...
    func subscribeToNotifications(completion: @escaping (Error?) -> Void) {
        ...
        let subscription = CKQuerySubscription(recordType: Message.messageTypeKey, predicate: predicate, options: .firesOnRecordCreation)
        ...
        publicDB.save(subscription) { (_, error) in
            if let error = error {
                print("Error saving subscription to public DB: \(error)")
